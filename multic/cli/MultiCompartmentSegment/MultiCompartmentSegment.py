...
def main(args):
    ...
    # In Girder worker runs this is mounted under /mnt/girder_worker/<worker_id>
    # but when running locally that path may not exist. Fall back to a
    # local temporary worker directory to allow local testing.
    try:
        if (
            os.path.isdir("/mnt/girder_worker")
            and len(os.listdir("/mnt/girder_worker")) > 0
        ):
            mounted_path = os.path.join(
                "/mnt/girder_worker", os.listdir("/mnt/girder_worker")[0]
            )
        else:
            raise FileNotFoundError
    except Exception:
        # fallback local worker path inside the repo working directory
        local_tmp = os.path.join(os.getcwd(), "tmp_girder_worker", "worker")
        os.makedirs(local_tmp, exist_ok=True)
        mounted_path = local_tmp

    file_path = os.path.join(mounted_path, file_name)
