...
LABEL last_updator="Ishaq Ansari - Sarder Lab. <iansari@ufl.edu>"
...
# Set TMPDIR and pip cache directory to a predictable location inside the image
# so pip's temporary unpack directories are located in /tmp and can be removed
# in the same layer. This reduces build-time disk pressure.
ENV TMPDIR=/tmp \
    PIP_CACHE_DIR=/tmp/pip_cache

RUN mkdir -p /tmp /tmp/pip_cache && chmod 1777 /tmp
...
RUN \
    # Use TMPDIR and PIP_CACHE_DIR so pip/unpack temporary files are predictable
    TMPDIR=/tmp PIP_CACHE_DIR=/tmp/pip_cache \
    pip install --no-cache-dir --upgrade --ignore-installed pip setuptools==69.5.1 && \
    TMPDIR=/tmp PIP_CACHE_DIR=/tmp/pip_cache pip install --no-cache-dir . && \
    TMPDIR=/tmp PIP_CACHE_DIR=/tmp/pip_cache pip install --no-cache-dir tensorboard cmake onnx && \
    TMPDIR=/tmp PIP_CACHE_DIR=/tmp/pip_cache pip install --no-cache-dir torch==1.10 torchaudio==0.10 torchvision==0.11.1 -f https://download.pytorch.org/whl/cu111/torch_stable.html && \
    TMPDIR=/tmp PIP_CACHE_DIR=/tmp/pip_cache python -m pip install detectron2 -f https://dl.fbaipublicfiles.com/detectron2/wheels/cu111/torch1.10/index.html && \
    # Clean up pip cache and temporary unpack directories in the same RUN so they
    # do not persist into intermediate layers
    rm -rf /root/.cache/pip/* /tmp/pip-* /tmp/pip_build* /tmp/pip-unpack-* /tmp/* || true
