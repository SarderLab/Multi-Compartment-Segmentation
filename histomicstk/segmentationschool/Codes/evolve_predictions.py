...
Saves a .gif image of the predictions from the specified region
...
def evolve(args):
    ...
    if iteration == 'none':
        ...
        for iter in range(1,iteration+1):
            dirs['xml_save_dir'] = args.base_dir + '/' + args.project + dirs['validation_data_dir'] + 'evolved_XMLs/'
            ...
            for wsi in WSIs:
                # predict xmls
                predict_all_xmls(args=args, dirs=dirs, wsi=wsi, iteration=iter)
...
def get_all_test_steps(modeldir):
    pretrains=glob(modeldir + '/*.ckpt*')
    model_IDs = []
    for modelfiles in pretrains:
        modelID=modelfiles.split('.')[-2].split('-')[1]
        model_IDs.append(int(modelID))
    model_IDs = np.sort(np.unique(np.array(model_IDs)))
    print('Found models: ')
    print(model_IDs[1:])
...
def crop_suey(label_offsets, dirs, args, classNum, iter, idx):
    ...
    for line in range(0, np.size(lines)):
        ...
        for value in np.unique(mask_image)[1:]:
            ...
            pointsList = get_contour_points(binary_mask, args=args, offset=label_offset)
    ...
    # save xml
    xml_save(Annotations=Annotations, filename=dirs['xml_save_dir']+'/'+dirs['fileID']+'_'+str(iter)+'_'+str(idx)+'.xml')

def get_contour_points(mask, args, offset={'X': 0,'Y': 0}):
    # returns a dict pointList with point 'X' and 'Y' values
    # input greyscale binary image
    _, maskPoints, contours = cv2.findContours(np.array(mask), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    pointsList = []
