...
def savetime(args, starttime):
    ...
    if args.option in ['train', 'Train']:
        with open(args.base_dir +  '/runtime.txt', 'a') as timefile:
            timefile.write('\n' + args.option +'\t'+ str(time.time()-starttime) +'\t'+ str(args.epoch_LR) +'\t'+ str(args.epoch_HR) +'\t'+ str(args.aug_LR) +'\t'+ str(args.aug_HR) +'\t'+ str(args.overlap_percentLR))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_steps', dest='train_steps', default=200 ,type=int,
        help='Size of batches for training high resolution CNN')
    #Hyperparameters
    #Hyperparameters
    ...
    parser.add_argument('--outDir', dest='outDir', default='/output' ,type=str,
