...
def predict(args):
    ...
    downsample = int(args.downsampleRateHR**0.5)
    ...
    cfg.MODEL.ANCHOR_GENERATOR.ASPECT_RATIOS = [
        [0.1, 0.2, 0.33, 0.5, 1.0, 2.0, 3.0, 5, 10]
    ]
    ...
    with tqdm(
        total=totalpatches, unit="image", colour="green", desc="Total WSI progress"
    ) as pbar:
        for i, j in coordinate_pairs(index_y, index_x):
            ...
            pbar.update(1)
            # DEBUG: Stop after 100 patches for testing
            # if pbar.n >= 100:
            #     break
            if np.sum(binary[yStart_small:yStop_small, xStart_small:xStop_small]) > (
                args.white_percent * box_total
            ):
                ...
                maskpart = decode_panoptic(
                    panoptic_seg.to("cpu").numpy(), segments_info, "kidney", args
                )
                if dxE != dim_x:
                ...
                wsiMask[dyS:dyE, dxS:dxE] = np.maximum(
                    maskpart, wsiMask[dyS:dyE, dxS:dxE]
                )
    ...
    for value in np.unique(unique_mask)[1:]:
        ...
        for i in range(len(pointsList)):
            ...
            Annotations = xml_add_region(
                Annotations=Annotations, pointList=pointList, annotationID=value
            )
    gc = args.gc
    annots = convert_xml_json(Annotations, NAMES)
    for annot in annots:
        # Post the annotation to Girder
        created = gc.post(
            path="annotation",
        ...
        print(
            "uploaded layer, server response id:",
            created.get("_id") or created.get("id"),
        )
        # Fetch the created annotation back from server to get element ids
        ann_id = created.get("_id") or created.get("id")
        if ann_id:
            server_ann = gc.get("annotation/%s" % ann_id)
            # server_ann['annotation']['elements'] contains element objects with server-assigned ids
            changed = False
            for el in server_ann.get("annotation", {}).get("elements", []):
                # element id may be under 'id' or '_id'
                el_id = el.get("id") or el.get("_id")
                if el_id and (
                    not el.get("label") or el.get("label", {}).get("value") != el_id
                ):
                    el["label"] = {"value": el_id}
                    changed = True
            if changed:
                # PUT the updated annotation back
                updated = gc.put(
                    "annotation/%s" % ann_id, data=json.dumps(server_ann["annotation"])
                )
                print("updated annotation to set element labels")
        else:
            print(
                "warning: server did not return an annotation id; cannot set element labels"
            )
...
def get_contour_points(mask, args, downsample, value, offset={"X": 0, "Y": 0}):
    ...
    maskPoints, contours = cv2.findContours(
        np.array(mask), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS
    )
    pointsList = []
        ...
        if len(maskPoints[j]) > 2:
            if cv2.contourArea(maskPoints[j]) > args.min_size[value - 1]:
                ...
                for i in np.array(range(0, len(maskPoints[j]), 4)):
                    point = {
                        "X": (maskPoints[j][i][0][0] * downsample) + offset["X"],
                        "Y": (maskPoints[j][i][0][1] * downsample) + offset["Y"],
                    }
...
def xml_add_region(
    Annotations, pointList, annotationID=-1, regionID=None
    ...
    for point in pointList:  # add new Vertex
        ET.SubElement(
            Vertices,
            "Vertex",
            attrib={"X": str(point["X"]), "Y": str(point["Y"]), "Z": "0"},
        )
    # add connecting point
    ET.SubElement(
        Vertices,
        "Vertex",
        attrib={"X": str(pointList[0]["X"]), "Y": str(pointList[0]["Y"]), "Z": "0"},
    )
    return Annotations
