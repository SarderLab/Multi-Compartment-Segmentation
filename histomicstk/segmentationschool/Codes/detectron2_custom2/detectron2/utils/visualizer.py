...
def mask2polygons(mask):
    ...
    if 7 in presentclasses:
        print('Beware, this mask directory has level 7 annotations in it!')
...
class Visualizer:
    ...
    def __init__(self, img_rgb, metadata=None, scale=1.0,idgen=None, instance_mode=ColorMode.IMAGE):
        """
        Args:
    ...
    def draw_dataset_dict(self, dic,train_dset):
        """
        ...
        x=train_dset.get_xml_folder()+'_'.join(dic['image_id'].split('_')[:-2])+'.xml'
        # print(x)
        # USER: Write your own image loading if it's not from a file
        # image = utils.read_image(dic["file_name"], format=self.image_format)
        # image=np.array(train_dset.get_single_openslide(dic['slide_index']).read_region((c[0],c[1]),0,(h,w)))
        maskData=xml_to_mask(x, c, [h,w])
        annos=mask2polygons(maskData)
        ...
        sem_seg = np.copy(np.array(maskData)).astype('uint8')
        sem_seg[sem_seg>2]=0
        sem_seg[maskData==0] = 3
        ...
        annos=mask2polygons(maskData)
        segments_info,pan_seg=get_seg_info(maskData,self.IdGen)
